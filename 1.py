...
def onclick(event: MouseEvent):
    ...
    if event.inaxes is not None:  # Check if the click is inside the axes
        ...
        if x is not None and y is not None:
            ...
            # Prompt user for label using a dialog box
            root = tk.Tk()
            root.withdraw()  # Hide the main window

            label = simpledialog.askinteger("Input", f"Enter label for point {click_index} (1 or 0):", minvalue=0, maxvalue=1)
            if label is not None:
                point_labels.append(label)
                print(f"Label for point {click_index}(1 or 0): {label}")
            else:
                print(f"No label entered for point {click_index}.")
                point_labels.append(-1)  # Append -1 to indicate no label entered
...
def setup_args(parser):
    parser.add_argument(
        ...
        default="D:/medical_paper/Inpaint-Anything/example/bmode1.jpg",
...
if __name__ == "__main__":
    ...
    if args.coords_type == "click":
        ...
        # Assign latest_coords after the interaction
        latest_coords = np.array(clicked_points)
        # Check if all points have labels
        if len(point_labels) != len(clicked_points):
            print("Some points are missing labels. Please ensure all points are labeled.")
    ...
    if latest_coords is not None:
        ...
        raise ValueError("No valid coordinates provided. Exiting...")